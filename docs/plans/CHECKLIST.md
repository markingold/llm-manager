...
- Historical references:
	- [docs/plans/retired/WEBUI_OVERHAUL_CHECKLIST.md](docs/plans/retired/WEBUI_OVERHAUL_CHECKLIST.md)
	- [docs/plans/retired/WEBUI_OVERHAUL_PLAN.md](docs/plans/retired/WEBUI_OVERHAUL_PLAN.md)
	- [docs/plans/retired/WEBUI_OVERHAUL_CLOSEOUT.md](docs/plans/retired/WEBUI_OVERHAUL_CLOSEOUT.md)
- New UI work should be tracked in this master checklist and the roadmap below.

Source-plan retirement update (2026-05-15):
...
- [x] Implement provider adapters for local, OpenRouter, and OpenAI. - effort: large | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (chat adapter slice implemented)
- [x] Add `POST /router/chat` as the first universal broker endpoint. - effort: medium | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (initial dry-run broker slice)
- [x] Persist provider runtime state for health, cooldowns, rate limits, queue state, and evaluation artifacts. - effort: large | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md, docs/plans/retired/EVALUATION_PLAN.md (evaluation_suites/runs/reports state persisted)
- [x] Add a shared OpenRouter free-tier limiter capped at 20 requests per minute. - effort: medium | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (local protective limiter slice implemented)
- [x] Add queueing for free-tier requests with configurable overflow behavior. - effort: large | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (wait/fail_fast/fallback_to_local/upgrade_to_paid slice implemented)
- [x] Add automatic smoke-check promotion and lifecycle state transitions for discovered OpenRouter free candidates. - effort: medium | source: docs/plans/retired/OPENROUTER_FREE_MODEL_DISCOVERY_PLAN.md
- [x] Persist richer OpenRouter discovery ranking fields and lifecycle/smoke evidence surfacing (`top_weekly_rank`, `category_ranks`, transition/smoke evidence fields). - effort: medium | source: docs/plans/NEXT_PUSH.md, docs/plans/retired/OPENROUTER_FREE_MODEL_DISCOVERY_PLAN.md
...
- [x] Add a TabbyAPI launcher and treat EXL2 and EXL3 as first-class ExLlama-backed lanes. - effort: large | source: docs/plans/BACKEND_ARCHITECTURE_PLAN.md, docs/plans/EXLLAMA_CONVERSION_PLAN.md (launcher wrapper plus backend-aware local lane dispatch and endpoint resolution are now in place)
- [x] Add OpenRouter metadata refresh, error normalization, cooldown state, and automatic free-model cycling. - effort: large | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (including status/body-aware error normalization, configurable cooldown policy knobs, lifecycle states, and capability-aware smoke checks)
- [x] Add controlled cross-provider fallback strategies such as `local_first`, `free_first`, `paid_first`, `best_available`, and `strict_provider`. - effort: large | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (including service-tier-aware chain resolution, strict-provider lane targeting, and typed fallback reason codes)
- [x] Support explicit provider selection and explicit model selection from client projects. - effort: medium | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (`provider_preferences.preferred_provider` + `model_preferences.preferred_model` contract and strict-provider lane execution)
- [x] Add evaluation suite schemas, storage, runner, and reporting endpoints. - effort: large | source: docs/plans/retired/EVALUATION_PLAN.md (initial local evaluation endpoint and report endpoints implemented)
- [x] Write external-project documentation for routed inference and evaluation-suite submission. - effort: medium | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md, docs/plans/retired/EVALUATION_PLAN.md, docs/TODO/README.md (`docs/guides/EXTERNAL_INTEGRATION.md` added)
...
- [x] Add router observability endpoints for health, last decisions, fallback stats, usage summaries, queue state, and evaluation summaries. - effort: medium | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md, docs/plans/retired/EVALUATION_PLAN.md (evaluation summary endpoint implemented)
- [x] Add cost-aware dynamic model ranking within a strategy (beyond fixed lane order). - effort: medium | source: docs/plans/NEXT_PUSH.md, docs/plans/retired/PROVIDER_ROUTER_PLAN.md (dynamic lane ranking now uses policy-tunable cost/availability/quality weights and surfaces ranking diagnostics in chain resolution)
- [x] Surface backend, model format, and recommendation data in the dashboard. - effort: medium | source: docs/plans/BACKEND_ARCHITECTURE_PLAN.md (slot-level backend/model/recommendation pills now shown in Operations cards)
- [x] Hide or disable unsupported operations per backend in the UI. - effort: small | source: docs/plans/BACKEND_ARCHITECTURE_PLAN.md (backend-aware test gating now disables unsupported or mismatched slot test actions with explicit reasons)
- [x] Add admin controls for enabling, disabling, prioritizing, and testing curated provider models. - effort: medium | source: docs/plans/retired/PROVIDER_ROUTER_PLAN.md (`/providers/models/curated-summary`, `/providers/models/curated-entry`, dashboard quick-admin grid)
...
- [x] Support rerunning historical evaluation suites against new candidate models. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (initial suite rerun endpoint implemented for local pipeline)
- [x] Generate compact comparison artifacts suitable for ChatGPT or Copilot adjudication. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (initial compare-compact endpoint implemented)
- [x] Add async evaluation queueing with configurable priority classes for local batch runs. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (interactive/batch/evaluation queue slice implemented)
- [x] Add per-priority concurrency caps and multi-worker execution controls for local evaluation queues. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (worker/cap controls implemented)
- [x] Add pluggable scoring checks for local evaluation cases (regex/json/contains/length) to improve prompt and parameter tuning decisions. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (initial scoring plugin slice implemented)
- [x] Add threshold-based pass/fail gating and filtered evaluation run listing for tighter tuning audits. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (case/suite thresholds and `/router/evaluations` filtering implemented)
- [x] Run local evaluation suites against mixed local and remote candidates with provider/lane-aware compare artifacts. - effort: medium | source: docs/plans/retired/EVALUATION_PLAN.md (mixed candidate execution implemented)
